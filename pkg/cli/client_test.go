...
const (
	...
	SecurePort3    = "8090"
	BaseURL4       = "http://127.0.0.1:8091"
	BaseSecureURL4 = "https://127.0.0.1:8091"
	SecurePort4    = "8091"
	username       = "test"
	passphrase     = "test"
	...
	certsDir3      = "/.config/containers/certs.d/127.0.0.1:8090/"
)
...
func TestTLSWithoutCerts(t *testing.T) {
	Convey("Set server TLS", t, func() {
		...
		Convey("Make first controller", func() {
			config := api.NewConfig()
			...
			Convey("Test without certs in user's home", func() {
				configPath := makeConfigFile(
					fmt.Sprintf(`{"configs":[{"_name":"imagetest","url":"%s/v2/_catalog","showspinner":false}]}`,
						BaseSecureURL3))
				defer os.Remove(configPath)
				...
				home := os.Getenv("HOME")
				destCertsDir := filepath.Join(home, certsDir3)
		...
		Convey("Make second controller", func() {
			config := api.NewConfig()
			...
			Convey("Test without certs in /etc/containers/certs.d/$IP:$PORT", func() {
				configPath := makeConfigFile(
					fmt.Sprintf(`{"configs":[{"_name":"imagetest","url":"%s/v2/_catalog","showspinner":false}]}`,
						BaseSecureURL4))
				defer os.Remove(configPath)
