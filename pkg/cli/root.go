					...
					for {
						...
						case event := <-watcher.Events:
							if event.Op == fsnotify.Write {
								...
								ctrlr.Config.AccessControl = newConfig.AccessControl
							}
						// watch for errors
	...
	scrubCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err == nil {
				...
				if err := ctrlr.InitImageStore(); err != nil {
					panic(err)
				}

				result, err := ctrlr.StoreController.CheckAllBlobsIntegrity()
				if err != nil {
					panic(err)
				}
