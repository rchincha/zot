...
* Authentication via:
  ...
  * HTTP *Bearer* token
* [Authorization](#Authorization) [Access Control List](./examples/config-policy.json)
...
* Supports live modifications on the config file while zot is running
* Supports verification of config files:
```console
$ zot verify /path/to/config
```

Zot follows the next logic for deciding who has rights on what resource:
1) First it checks the requested resource in "repositories"
2) Searches in all policies for the user who requested the resource
3) If found, applies the policy, otherwise it falls back to the default policy for that resource
